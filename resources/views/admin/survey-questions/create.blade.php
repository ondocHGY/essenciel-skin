            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="category" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        <option value="basic" {{ old('category') == 'basic' ? 'selected' : '' }}>기본 정보 (연령, 성별 등)</option>
                        <option value="lifestyle" {{ old('category', 'lifestyle') == 'lifestyle' ? 'selected' : '' }}>생활 습관 (수면, 스트레스 등)</option>
                        <option value="habit" {{ old('category') == 'habit' ? 'selected' : '' }}>케어 습관 (스킨케어 등)</option>
                    </select>
                </div>
            </div>
...
function questionForm() {
    return {
        options: [
            { value: '', label: '', modifier: 1.0, description: '' },
            { value: '', label: '', modifier: 1.0, description: '' },
        ],
        addOption() {
            this.options.push({ value: '', label: '', modifier: 1.0, description: '' });
        },
        removeOption(index) {
